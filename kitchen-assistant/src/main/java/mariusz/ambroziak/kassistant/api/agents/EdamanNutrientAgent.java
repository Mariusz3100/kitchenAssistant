...
public class EdamanNutrientAgent extends BaseAgent{

	static ArrayList<EdamanNutrientAgent> agents;
	...
	protected void activate() {
		...
		if(agents==null)agents=new ArrayList<EdamanNutrientAgent>();
		agents.add(this);
	...
	public static ProduktWithAllIngredients parseFullSklad(String shortUrl)
		...
		EdamanNutrientAgent freeOne = getFreeAgent();
		if(freeOne==null){
			return null;
		}else{
...
//		return produktWithIngredinets;
	...
	private static EdamanNutrientAgent getFreeAgent() throws AgentSystemNotStartedException {
		EdamanNutrientAgent freeOne=null;
		...
		if(agents==null){
			...
			while(freeOne==null){
				for(EdamanNutrientAgent ra:agents){
					if(!ra.isBusy()){
						freeOne=ra;
					}
				}
...
//			ProduktWithBasicIngredients basics = EdamanNutrientAgent.parseBasicSklad(sr.getProdukt().getUrl());
...
//	for(SingleProdukt_SearchResult sr:goodResults){
//		ProduktWithAllIngredients basics = EdamanNutrientAgent.parseFullSklad(sr.getProdukt().getUrl());
//		retValue.put(sr, basics);
//	}
//	return retValue;
//}
